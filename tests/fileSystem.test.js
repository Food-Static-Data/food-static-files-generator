...
// i.e. instead of passing simple array into test, we can actually run tests with our real files/objects
...
describe("testing function isFolderExists()", () => {
  ...
  test("test isFolderExists() returns false", async () => {
    ...
    // some important folders(few), like `src` or `dist`(sometimes dist are actually removed, cleaned up)
